...
int main(void) {
    ...
    *(PS2_ptr) = 0xFF;  // Reset PS/2 port
    ...
    /*
    This section polls SW0 to allow the program to know when the electricity timer is operating 
    */

    volatile int* switch_ptr = (int*) SW_BASE;
    //int SW0 = *switch_ptr & 0x1;
    int SW9 = *switch_ptr & 0x200;

        if (SW9) {
            drawText("ON", 100, 100, 0xFFFF);
        } else {
            drawText("ON", 100, 100, 0x0000);  // Overwrite "ON" with black
        }
    ...
    // Iterate through each switch/LED pair (0-8 corresponds to switches 1-9)
    for (int i = 0; i < 9; i++) {
        // Check if current switch is ON
        int switch_state = (sw_value >> i) & 0x1;
        SW_on_off[i] = switch_state;  // Update switch state array
